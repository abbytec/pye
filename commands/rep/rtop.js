...
module.exports = {
    ...
    async execute(msg, [page]) {
        ...
        const position = allDocs.findIndex((u) => u._id === msg.author.id)
        const content = async (disable = false) => {
            let users = await HelperPoint.find().sort({ points: -1 }).skip(page * 10).lean().exec()
            ...
            return {
                embeds: [new EmbedBuilder()
                    ...
                    .addFields([{
                        ...
                        value: (await Promise.all(users.map(async (u, i) => {
                            const member = await msg.guild.members.fetch(u._id).catch(() => ({ user: { username: u._id } }))
                            return `**${(page * 10) + i + 1}.** [${member.user.username}](https://discord.com/users/${u._id}) • ${u.points.toLocaleString()} puntos.`
                        }))).join('\n') || 'No hay usuarios en el top.'
                    },
                    {
                        name: 'Tu posición',
                        value: !isNaN(position) ? (position + 1) === 0 ? 'No te encontré en el top.' : '#' + (position + 1) : 'No te encontré en el top.'
                    }])
        ...
        let m = await msg.channel.send(await content()).catch(() => null)

        m?.createMessageComponentCollector({
            filter: (i) => i.user.id === msg.author.id && ['hp-topBack', 'hp-topNext'].includes(i.customId),
            time: 60e3,
        }).on('collect', async (i) => {
            if (i.customId === 'hp-topBack' && page >= 0)
                page--
            else if (i.customId === 'hp-topNext' && page < all)
                page++
            else
                i.deferUpdate()
