...
import {
  ...
  MessageToSend,
  ParameterError,
} from "../../interfaces/IPrefixChatInputCommand.js";
...
export class PrefixChatInputCommand {
  ...
  constructor(
    ...
    commandName: string,
    argsDefinition: IPrefixChatInputCommandOption[],
    aliases: string[] = [],
  ) {
    this.client = client;
  ...
  public async parseMessage(
    message: Message,
  ): Promise<IPrefixChatInputCommand | null> {
  ...
  private readonly getString = (
    ...
    if (hasMarkdown) {
      ...
      if (match) messageContent = match[2];
    ...
    if (required && (val === undefined || val === null)) {
      throw new ParameterError(
        `El argumento requerido "${name}" no fue proporcionado.`,
      );
    }
  ...
  private readonly getNumber = (
    ...
    if (isNaN(num)) {
      if (required)
        throw new ParameterError(
          `El argumento "${name}" no es un número válido.`,
        );
      return null;
    }
    return num;
  ...
  private async getAttachment(
    ...
    if (!val) {
      if (required) {
        throw new ParameterError(
          `El archivo adjunto requerido "${name}" no fue proporcionado.`,
        );
      }

      if (this.message?.reference) {
        const repliedMessage = await this.message.channel.messages
          .fetch(this.message.reference.messageId ?? "")
          .catch(() => undefined);
    ...
    try {
      ...
      if (required) {
        throw new ParameterError(
          `El archivo adjunto requerido "${name}" no fue proporcionado correctamente.`,
        );
      }
    }
  ...
  private readonly getInteger = (
    ...
    if (isNaN(int)) {
      if (required)
        throw new ParameterError(
          `El argumento "${name}" no es un número entero válido.`,
        );
      return null;
    }
    return int;
  ...
  private async getRole(
    ...
    try {
      ...
      if (!role && required) {
        throw new ParameterError(
          `No se pudo encontrar el rol para el argumento "${name}".`,
        );
      }
      return role ?? null;
    } catch {
  ...
  private async getChannel(
    ...
    try {
      ...
      if (!channel && required) {
        throw new ParameterError(
          `No se pudo encontrar el canal para el argumento "${name}".`,
        );
      }
      return channel ?? null;
    } catch {
  ...
  private readonly getSubcommand = (required?: boolean): string | null => {
    ...
    if (required && (!subCommand || subCommand.trim() === "")) {
      throw new ParameterError(
        `El subcomando es requerido pero no fue proporcionado.`,
      );
    }
  ...
  private async reply(content: MessageToSend): Promise<Message> {
    if (!this.message)
      throw new ParameterError("No se ha parseado un mensaje aún.");
  ...
  private async deleteReply(): Promise<void> {
    if (!this._reply)
      throw new ParameterError("No hay una respuesta previa para borrar.");
  ...
  private async followUp(content: MessageToSend): Promise<Message> {
    if (!this.message?.channel)
      throw new ParameterError("No se ha parseado un mensaje aún.");
    if (content instanceof Object && "ephemeral" in content) {
      return (this.message.channel as TextChannel).send({
        ...content,
        options: { ephemeral: content.ephemeral },
      } as any);
    } else {
