...
export default {
	data: new SlashCommandBuilder()
		...
		.addStringOption((option) => option.setName("url").setDescription("Enlace para analizar"))
	...
	execute: composeMiddlewares(
		...
		async (interaction: IPrefixChatInputCommand): Promise<PostHandleable | void> => {
			...
			try {
				...
				embed
					...
					.setDescription(
						`🔗 **[Ver reporte en VirusTotal](${reportUrl})**\n` +
							`📊 **Total de Antivirus:** ${
								analysisStats.malicious + analysisStats.suspicious + analysisStats.harmless + analysisStats.undetected
							}\n` +
				...
				embed.setColor(COLORS.errRed).setTitle("Error").setDescription(`Fallo al escanear: ${error.message}`);
			}
...
async function pollScanResults(scanId: string, retries: number, delay: number): Promise<VirusTotalAnalysisResult> {
	for (let attempt = 0; attempt < retries; attempt++) {
		try {
			...
			if (
				!stats ||
				typeof stats.malicious === "undefined" ||
				typeof stats.suspicious === "undefined" ||
				typeof stats.harmless === "undefined" ||
				typeof stats.undetected === "undefined"
			) {
...
function generateReportUrl(attachment: Attachment | null, scanResult: VirusTotalAnalysisResult): string {
	...
	return `https://www.virustotal.com/gui/${isFile ? "file" : "url"}/${
		isFile ? scanResult.meta.file_info.sha256 : scanResult.meta.url_info.id
	}`;
}
