...
export default {
    ...
    async (
      ...
      try {
        ...
        embed
          ...
          .setTitle("❌ Error")
          .setDescription(
            `No se pudo obtener la información del VPS: ${error.message}`,
          );
      }
  ...
  prefixResolver: (client: ExtendedClient) =>
    new PrefixChatInputCommand(client, "usage", [], []),
};
  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < start.length; i++) {
        ...
        const total =
          Object.values(endCore.times).reduce((a, b) => a + b, 0) -
          Object.values(startCore.times).reduce((a, b) => a + b, 0);
