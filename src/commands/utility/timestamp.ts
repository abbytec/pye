...
const ZONE_CHOICES = [
	{ name: "UTC", value: "UTC" },
	{ name: "UTC-3 Argentina / Uruguay / Paraguay", value: "UTC-3" },
	{ name: "UTC-4 Chile / Venezuela", value: "UTC-4" },
	{ name: "UTC-5 Colombia / Perú", value: "UTC-5" },
...
export default {
	data: new SlashCommandBuilder()
		...
		.addStringOption((option) => option.setName("fecha").setDescription("Fecha y hora (YYYY-MM-DD HH:mm)").setRequired(true))
	...
	execute: composeMiddlewares([deferInteraction(false)], async (interaction: IPrefixChatInputCommand): Promise<PostHandleable | void> => {
		...
		if (!dt.isValid) {
			return replyError(interaction, "Fecha inválida. Usa el formato YYYY-MM-DD HH:mm");
		}

		const timestamp = Math.floor(dt.toSeconds());
		const result = `\`<t:${timestamp}:${type}>\` = <t:${timestamp}:${type}>`;

		await replyOk(interaction, result, null, undefined, undefined, undefined, false);
	}),
} as Command;
